...
def convert_docstring_sections(
    ...
    """Parse docstring into (desc, param_docs, return_desc, complexity)."""
    lines = text.split("\n")
    result = []
    ...
    while i < len(lines):
        ...
        if stripped.startswith("Returns:") or stripped.startswith("Return:"):
            ...
            in_returns = True
            in_raises = False
            return_desc = convert_sphinx(stripped.split(":", 1)[1].strip())
            i += 1
            continue
        ...
        if in_returns:
            ...
            if stripped_desc.startswith(":") and ":" in stripped_desc[1:]:
                in_returns = False
                continue
            return_desc += " " + convert_sphinx(line.rstrip())
            i += 1
            continue
        ...
        if m:
            return_desc = convert_sphinx(m.group(1))
            i += 1
            while i < len(lines):
                nxt = lines[i]
                nxt_stripped = nxt.strip()
                if (
                    ...
                    return_desc += " " + convert_sphinx(nxt_stripped)
                    i += 1
                else:
                    break
            continue
    ...
    return str("\n".join(result)), param_docs, return_desc, complexity
...
def render_members(members: dict, mod_doc: str | None) -> str:
    ...
    if has_alias:
        ...
        for a in public_aliases:
            ...
            lines.append("")
            doc, _, _, _ = format_docstring(a["doc"])
            if doc:
                lines.append(doc)
                lines.append("")
        lines.append("")
    ...
    if has_consts:
        ...
        for c in public_consts:
            ...
            if c["type"]:
                ...
                lines.append("")
            doc, _, _, _ = format_docstring(c["doc"])
            if doc:
                lines.append(doc)
                lines.append("")
        lines.append("")
        ...
        for cls in sorted(members["classes"], key=lambda x: x["name"]):
            ...
            for p in public_props:
                ...
                if sig:
                    ...
                    lines.append("")
                pd, _, _, _ = format_docstring(p["doc"])
                if pd:
                    lines.append(pd)
                    lines.append("")
            ...
            for m in public_methods:
                ...
                lines.append("")
                mdoc, param_docs, m_return_desc, m_complexity = (
                    format_docstring(m["doc"])
                )
                if mdoc:
                    lines.append(mdoc)
                    lines.append("")
            ...
            for nc in public_nested:
                ...
                lines.append("")
                ndoc, _, _, _ = format_docstring(nc["doc"])
                if ndoc:
                    lines.append(ndoc)
                    lines.append("")
                ...
                for m in nc_public_methods:
                    ...
                    lines.append("")
                    lines.append(
                        _format_sig(f"{dec}{m['name']}{m['signature']}")
                    )
                    lines.append("")
                    mdoc, param_docs, m_return_desc, m_complexity = (
                        format_docstring(m["doc"])
    ...
    if has_funcs:
        ...
        for f in sorted(members["functions"], key=lambda x: x["name"]):
            ...
            lines.append("")
            doc, param_docs, f_return_desc, f_complexity = format_docstring(
                f["doc"]
            )
            if doc:
                lines.append(doc)
