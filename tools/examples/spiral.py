...
def generate_examples(output_dir):
    ...
    fig.tight_layout()
    path = output_dir / "spiral-inward-outward.png"
    fig.savefig(path, dpi=150)
    plt.close(fig)
    images.append(
        {
            "path": "spiral-inward-outward.png",
            "caption": "Outward (CCW) and inward (CW) flat Archimedean spirals",  # noqa: E501
        }
    )

    return {
        "title": "Flat Archimedean Spiral Generation",
        "description": (
            "Generate constant-Z spiral paths for helical entry widening "
            "and similar toolpath operations."
        ),
        "images": images,
    }
