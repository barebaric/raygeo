"""Visualise deepest-core detection for adaptive clearing."""
...
__images__ = [
    ...
    {
        "stem": "deepest-cores-multi-island",
        "caption": (
    ...
    {
        "stem": "deepest-cores-central-island",
        "caption": (
...
def generate_examples(output_dir):
    ...
    # Right: L-shaped pocket — core at the lobe intersection (deepest point)
    ...
    fig.tight_layout()
    path = output_dir / "deepest-cores.png"
    fig.savefig(path, dpi=150)
    plt.close(fig)
    images.append(
        {
            "path": "deepest-cores.png",
            "caption": (
                "Iterative inward offset shrinking a pocket until collapse;"
                " the centroid(s) of the final surviving polygon(s) mark"
                " optimal helical entry points."
            ),
        }
    )
    ...
    # Two islands: top-left and bottom-right
    ...
    # Left: valid tool area + islands + cores
    mb_arr = list(mb) + [mb[0]]
    ...
    # Right: offset sequences for each valid region, showing cores
    for poly in m_area:
    ...
    fig2.tight_layout()
    path2 = output_dir / "deepest-cores-multi-island.png"
    fig2.savefig(path2, dpi=150)
    plt.close(fig2)
    images.append(
        {
            "path": "deepest-cores-multi-island.png",
            "caption": (
                "The pocket boundary (black) contains two islands (grey)."
                " The valid tool area (blue dashed) is the boundary offset"
                " inward minus the islands offset outward.  find_deepest_cores"
                " returns a single core (red star) — the centroid of the"
                " largest surviving fragment across all valid regions."
            ),
        }
    )

    # ------------------------------------------------------------------
    # Central-island pocket (annular)
    # ------------------------------------------------------------------
    ...
    fig3.tight_layout()
    path3 = output_dir / "deepest-cores-central-island.png"
    fig3.savefig(path3, dpi=150)
    plt.close(fig3)
    images.append(
        {
            "path": "deepest-cores-central-island.png",
            "caption": (
                "A central island creates an annular valid tool area"
                " (blue dashed).  `find_deepest_cores` returns the core"
                " at the centre of the ring — the deepest accessible"
                " point in the pocket."
            ),
        }
    )
    ...
    return {
        "title": "Deepest-Core Detection",
        "description": (
            "Binary search finds the largest inward offset that does NOT"
            " collapse the pocket, then returns the centroid of the largest"
            " surviving fragment across all input polygons — the single"
            " best helical entry point for the entire pocket geometry."
        ),
        "images": images,
    }
