...
def generate_examples(output_dir):
    ...
    fig.tight_layout()
    path = output_dir / "trochoid-straight.png"
    fig.savefig(path, dpi=150)
    plt.close(fig)
    images.append(
        {
            "path": "trochoid-straight.png",
            "caption": "Trochoidal toolpath — 60° vs 120° engagement along straight carrier",  # noqa: E501
        }
    )
    ...
    fig.tight_layout()
    path = output_dir / "trochoid-l-shaped.png"
    fig.savefig(path, dpi=150)
    plt.close(fig)
    images.append(
        {
            "path": "trochoid-l-shaped.png",
            "caption": "Trochoidal toolpath around an L-shaped corner",
        }
    )

    return {
        "title": "Trochoidal Milling Paths",
        "description": (
            "Trochoidal toolpaths for constant-engagement milling, using "
            "a sinusoidal lateral offset along a carrier polyline."
        ),
        "images": images,
    }
