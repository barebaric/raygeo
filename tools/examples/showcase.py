...
def _plot_concave_hull(ax):
    ...
    if convex_geo is not None:
        plot_geometry(ax, convex_geo, color="tomato", label="Convex", linewidth=1.5)
    if concave_geo is not None:
        plot_geometry(ax, concave_geo, color="forestgreen", label="Concave", linewidth=2)
    ax.set_aspect("equal")
...
def _plot_arc_fitting(ax):
    ...
    cx, cy, r = 0, 0, 30
    arc_pts = [
        (cx + r * math.cos(math.pi * i / n_arc), cy + r * math.sin(math.pi * i / n_arc))
    ...
    plot_geometry(ax, fit_geom, color="forestgreen", linewidth=2.5, label=f"Fitted ({len(fit_geom)} cmds)")
    ax.set_aspect("equal")
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=8)
...
def _plot_nesting(ax):
    ...
    part_hulls = [[get_polygon_convex_hull(part_polys[i][0])] for i in range(n_parts)]
    sheet_poly = [(0.0, 0.0), (sheet_w, 0.0), (sheet_w, sheet_h), (0.0, sheet_h)]
    sheet_offsets = [(0.0, 0.0)]
    rotations = [0.0] * n_parts
    fh = [False] * n_parts
    fv = [False] * n_parts

    result = place_parts(part_polys, part_hulls, [sheet_poly], sheet_offsets, rotations, fh, fv, spacing=spacing)
...
def _plot_raster_power_modulation(ax):
    ...
    ops = rasterize_power_modulation(gray, alpha, (ppm, ppm), 0.0, 0.0, line_interval, 0.05, min_power=0.0, max_power=1.0, angle=0, scan_mode=ScanMode.Segmented)
    ...
    for i in range(ops.len()):
        ...
        if ct == CommandType.MOVE_TO:
            pos = ops.endpoint(i)
            continue
        if ct in (CommandType.SET_POWER, CommandType.SET_CUT_SPEED, CommandType.SET_TRAVEL_SPEED, CommandType.SET_LASER, CommandType.SET_FREQUENCY, CommandType.SET_PULSE_WIDTH, CommandType.ENABLE_AIR_ASSIST, CommandType.DISABLE_AIR_ASSIST):
            continue
        st = ops.state(i)
        pwr = st.power if st is not None and st.power is not None else 1.0
        if ct == CommandType.LINE_TO:
            ep = ops.endpoint(i)
            ax.plot([pos[0], ep[0]], [pos[1], ep[1]], color=power_cmap(pwr), linewidth=1.2)
            pos = ep
            ...
            if n > 0:
                ...
                power_arr = np.frombuffer(sd, dtype=np.uint8).astype(np.float64) / 255.0
                for j in range(n - 1):
                    ax.plot(xs[j:j+2], ys[j:j+2], color=power_cmap(power_arr[j]), linewidth=1.2)
            pos = ep
...
def _plot_smooth(ax):
    ...
    sx, sy = zip(*pts)
    ax.plot(sx + (sx[0],), sy + (sy[0],), color="gray", linewidth=1.5, label="Original")
    ssx, ssy, _ = zip(*smoothed)
    ax.plot(ssx + (ssx[0],), ssy + (ssy[0],), color="tomato", linewidth=2.5, label="Smoothed")
    ax.set_aspect("equal")
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=8)
...
def _plot_linearization(ax):
    ...
    ax.plot(fxs, fys, "-o", color="forestgreen", linewidth=2.5, markersize=3, label=f"tol=1 ({len(flat)} pts)")
    ax.set_aspect("equal")
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=8)
...
def generate_examples(output_dir):
    ...
    return {
        ...
        "images": [
            {
                "path": "showcase.png",
                "caption": "Concave hull, arc fitting, nesting, raster power modulation, smoothing, and linearization",
            }
        ],
    }
