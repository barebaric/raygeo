...
``geo`` — Pure geometry.
    ...
    Knows nothing about machining, motion commands, tools, or feed rates.
...
objects, never raw polygon or polyline lists. Motion classification is
encoded as ``MoveTo`` (travel) vs ``LineTo`` (cut) at the command level.
