...
class TestEvaluateBezier:
    def test_endpoints(self):
        ...
        assert get_bezier_point_at(p0, c1, c2, p1, 0.0) == approx_pt(p0)
        assert get_bezier_point_at(p0, c1, c2, p1, 1.0) == approx_pt(p1)

    def test_midpoint(self):
        p0, c1, c2, p1 = (0, 0), (0, 2), (4, 2), (4, 0)
        mid = get_bezier_point_at(p0, c1, c2, p1, 0.5)
    ...
    def test_line(self):
        ...
        pt = get_bezier_point_at(p0, c1, c2, p1, 0.5)
...
class TestSubdivideBezier:
    ...
    def test_split_at_one(self):
        p0, c1, c2, p1 = (0, 0), (1, 2), (3, 2), (4, 0)
        left, right = split_bezier(p0, c1, c2, p1, 1.0)
        assert left[0] == approx_pt(p0)
    ...
    def test_split_halves_reconstruct(self):
        ...
        for t in [0.0, 0.1, 0.25, 0.4, 0.5]:
            orig = get_bezier_point_at(p0, c1, c2, p1, t)
            from_left = get_bezier_point_at(*left, t * 2)
            assert from_left == approx_pt(orig)
        for t in [0.5, 0.6, 0.75, 0.9, 1.0]:
            orig = get_bezier_point_at(p0, c1, c2, p1, t)
            from_right = get_bezier_point_at(*right, (t - 0.5) * 2)
...
class TestBezierBounds:
    def test_straight_line(self):
        p0, c1, c2, p1 = (0, 0), (1, 0), (2, 0), (3, 0)
        bounds = get_bezier_bounds(p0, c1, c2, p1)
...
class TestIntersectBezierRect:
    def test_fully_inside(self):
        ...
        rect = (0, 0, 10, 10)
        params = get_bezier_rect_intersections(p0, c1, c2, p1, rect)
...
class TestClipBezier:
    def test_fully_inside(self):
        ...
        rect = (0, 0, 10, 10)
        segments = clip_bezier_with_rect(p0, c1, c2, p1, rect)
        assert len(segments) == 1
        start = get_bezier_point_at(*segments[0], 0.0)
        end = get_bezier_point_at(*segments[0], 1.0)
    ...
    def test_crosses_right_edge(self):
        ...
        assert len(segments) >= 1
        for seg in segments:
            end = get_bezier_point_at(*seg, 1.0)
    ...
    def test_crosses_two_opposite_edges(self):
        ...
        assert len(segments) >= 1
        for seg in segments:
            start = get_bezier_point_at(*seg, 0.0)
            end = get_bezier_point_at(*seg, 1.0)
            assert start[0] >= -1e-6
            assert end[0] >= -1e-6
    ...
    def test_clipped_segments_stay_inside(self):
        ...
        for seg in segments:
            for i in range(20):
                t = i / 19.0
                pt = get_bezier_point_at(seg[0], seg[1], seg[2], seg[3], t)
    ...
    def test_degenerate_point(self):
        p = (5, 5)
        segments = clip_bezier_with_rect(p, p, p, p, (0, 0, 10, 10))
        assert len(segments) == 1
    ...
    def test_start_outside_end_inside(self):
        ...
        segments = clip_bezier_with_rect(p0, c1, c2, p1, rect)
        assert len(segments) >= 1
        end = get_bezier_point_at(*segments[-1], 1.0)
        assert end == approx_pt(p1)
...
class TestIsBezierFullyInsideRegions:
    ...
    def test_fully_outside(self):
        region = self._square(0, 0, 10)
        p0, c1, c2, p1 = (12, 12), (13, 14), (15, 14), (16, 12)
        assert not is_bezier_inside_polygons(p0, c1, c2, p1, [region])
    ...
    def test_inside_one_of_multiple_regions(self):
        ...
        assert is_bezier_inside_polygons(p0, c1, c2, p1, [r1, r2])

    def test_spread_across_regions(self):
        r1 = self._square(0, 0, 5)
        r2 = self._square(10, 0, 5)
        p0, c1, c2, p1 = (2, 2), (5, 4), (8, 4), (12, 2)
        assert not is_bezier_inside_polygons(p0, c1, c2, p1, [r1, r2])

    def test_degenerate_point_inside(self):
        region = self._square(0, 0, 10)
        p = (5, 5)
        assert is_bezier_inside_polygons(p, p, p, p, [region])

    def test_degenerate_point_outside(self):
        region = self._square(0, 0, 10)
        p = (15, 15)
        assert not is_bezier_inside_polygons(p, p, p, p, [region])

    def test_empty_regions(self):
        p0, c1, c2, p1 = (2, 2), (3, 4), (5, 4), (6, 2)
        assert not is_bezier_inside_polygons(p0, c1, c2, p1, [])
