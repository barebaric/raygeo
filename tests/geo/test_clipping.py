...
def test_clip_line_segment_fully_inside(clip_rect):
    p1 = (10.0, 10.0, -1.0)
    p2 = (90.0, 90.0, -1.0)
    result = clip_line_segment_with_rect(p1, p2, clip_rect)
...
def test_subtract_polygons_from_line_segment():
    # A simple gap in the middle of a line
    p1 = (0.0, 50.0, -5.0)
    p2 = (100.0, 50.0, 5.0)
    region = [(40.0, 45.0), (60.0, 45.0), (60.0, 55.0), (40.0, 55.0)]

    kept_segments = subtract_polygons_from_line_segment(p1, p2, [region])
...
def test_clip_line_segment_to_regions_basic():
    ...
    kept_segments = clip_line_segment_with_polygons(p1, p2, [region])
...
def test_clip_line_segment_to_regions_multiple_regions():
    ...
    kept_segments = clip_line_segment_with_polygons(p1, p2, [region1, region2])
...
def test_clip_line_segment_to_regions_empty_regions():
    p1 = (0.0, 50.0, 0.0)
    p2 = (100.0, 50.0, 0.0)
    kept_segments = clip_line_segment_with_polygons(p1, p2, [])
    assert len(kept_segments) == 0
...
class TestClipToCircleRegions:
    ...
    def test_clip_to_circle_basic(self):
        ...
        kept_segments = clip_line_segment_with_polygons(p1, p2, [circle])
...
class TestSubtractCircleRegions:
    ...
    def test_subtract_circle_from_line(self):
        ...
        kept_segments = subtract_polygons_from_line_segment(p1, p2, [circle])
