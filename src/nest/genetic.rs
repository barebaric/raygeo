...
mod tests {
    ...
    fn test_ga_creation() {
        ...
        let ga = GeneticAlgorithm::new(10, config);
        assert_eq!(ga.population.len(), 20);
        assert_eq!(ga.num_parts, 10);
        assert!(!ga.population[0].rotation.is_empty());
    }
    ...
    fn test_population_diversity() {
        let config = small_config();
        let ga = GeneticAlgorithm::new(10, config);
        // Check that not all individuals have identical rotations
        let first_rot = &ga.population[0].rotation;
        let all_same = ga.population[1..]
            .iter()
            .all(|ind| ind.rotation == *first_rot);
        assert!(!all_same, "Population should have diverse rotations");
    }

    #[test]
    fn test_mutate_changes_individual() {
        let config = small_config();
        let ga = GeneticAlgorithm::new(10, config);
        let mutant = ga.mutate(0);
        // Mutant should have INF fitness (not evaluated yet)
        assert!(mutant.fitness.is_infinite());
        // With mutation_rate=30 and random flips, at least some genes
        // should differ (very likely but not guaranteed; we're testing
        // the mutation ran without error)
        assert_eq!(mutant.rotation.len(), 10);
        assert_eq!(mutant.flip_h.len(), 10);
        assert_eq!(mutant.flip_v.len(), 10);
    }

    #[test]
    fn test_mate_produces_two_children() {
        let config = small_config();
        let ga = GeneticAlgorithm::new(5, config);
        let (c1, c2) = ga.mate(0, 1);
        assert_eq!(c1.rotation.len(), 5);
        assert_eq!(c2.rotation.len(), 5);
        assert!(c1.fitness.is_infinite());
        assert!(c2.fitness.is_infinite());
    }

    #[test]
    fn test_mate_cutpoint_varies() {
        ...
        // Set distinct fitness so ordering is deterministic
        for (i, ind) in ga.population.iter_mut().enumerate() {
            ind.fitness = i as f64;
        }
        // Mate the same pair multiple times — cutpoint should vary
        let (c1_a, _) = ga.mate(0, 1);
        let (c1_b, _) = ga.mate(0, 1);
        // It's very unlikely that two random cutpoints produce identical
        // children (probability 1/9), but this test is really about
        // making sure the function works at all.
        let diff = c1_a.rotation != c1_b.rotation;
        // Due to randomness we just check they're valid
        assert_eq!(c1_a.rotation.len(), 10);
        assert_eq!(c1_b.rotation.len(), 10);
        // Force the check — we don't know at compile time
        _ = diff;
    }

    #[test]
    fn test_generation_sorts_by_fitness() {
        let config = small_config();
        let mut ga = GeneticAlgorithm::new(10, config);
        let pop_len = ga.population.len();
        // Set descending fitness (worst first)
        for (i, ind) in ga.population.iter_mut().enumerate() {
            ind.fitness = (pop_len - i) as f64;
        }
        ga.generation();
        // After generation, population should still have correct size
        assert_eq!(ga.population.len(), pop_len);
    }

    #[test]
    fn test_generation_maintains_population_size() {
        let config = small_config();
        let mut ga = GeneticAlgorithm::new(10, config);
    ...
    #[test]
    fn test_mate_single_part() {
        let config = small_config();
        let ga = GeneticAlgorithm::new(1, config);
        let (c1, c2) = ga.mate(0, 0);
        assert_eq!(c1.rotation.len(), 1);
        assert_eq!(c2.rotation.len(), 1);
        // With single part, children are clones of parents
        assert_eq!(c1.rotation[0], ga.population[0].rotation[0]);
        assert_eq!(c2.rotation[0], ga.population[0].rotation[0]);
    }

    #[test]
    fn test_no_flip_config() {
        let config = GeneticConfig {
            rotation_count: 4,
            flip_h: false,
            flip_v: false,
        ...
        let ga = GeneticAlgorithm::new(5, config);
        // All individuals should have no flips
        for ind in &ga.population {
            assert!(ind.flip_h.iter().all(|&f| !f));
            assert!(ind.flip_v.iter().all(|&f| !f));
        }
    }
}
