...
mod tests {
    ...
    fn test_floyd_steinberg_all_white() {
        let gray = vec![255u8; 10 * 10];
        let mut output = vec![0u8; 100];
        apply_floyd_steinberg_dither(&gray, 10, 10, false, &mut output);
        assert!(output.iter().all(|&v| v == 0));
    }

    #[test]
    fn test_floyd_steinberg_all_black() {
        let gray = vec![0u8; 10 * 10];
        let mut output = vec![0u8; 100];
        apply_floyd_steinberg_dither(&gray, 10, 10, false, &mut output);
    ...
    #[test]
    fn test_floyd_steinberg_invert() {
        let gray = vec![255u8; 10 * 10];
        let mut output = vec![0u8; 100];
        apply_floyd_steinberg_dither(&gray, 10, 10, true, &mut output);
        assert!(output.iter().all(|&v| v == 1));
    }

    #[test]
    fn test_minimum_run_length_noop() {
        let mut binary = vec![1u8, 1, 1, 0, 1, 1, 1];
        apply_minimum_run_length(&mut binary, 7, 1, 1);
        assert_eq!(binary, vec![1, 1, 1, 0, 1, 1, 1]);
    }

    #[test]
    fn test_minimum_run_length_removes_short() {
        let mut binary = vec![1u8, 1, 0, 1, 0, 1, 1, 1];
        apply_minimum_run_length(&mut binary, 8, 1, 3);
        assert_eq!(binary, vec![0, 0, 0, 0, 0, 1, 1, 1]);
    }

    #[test]
    fn test_bayer_dither_basic() {
        let gray = vec![128u8; 4 * 4];
        let bayer = vec![0.0f32, 2.0, 3.0, 1.0];
        let mut output = vec![0u8; 16];
        apply_bayer_dither(&gray, 4, 4, &bayer, 2, false, 1, &mut output);
        let ones: usize = output.iter().map(|&v| v as usize).sum();
