...
mod tests {
    ...
    fn test_optimize_empty() {
        let mut ops = Ops::new();
        optimize_travel(&mut ops, true, false, Vec::new(), None);
        assert!(ops.is_empty());
    }

    #[test]
    fn test_optimize_single_subpath() {
        let mut ops = Ops::new();
        ops.move_to(0.0, 0.0, 0.0, None);
        ops.line_to(10.0, 10.0, 0.0, None);
        let original_len = ops.len();
        optimize_travel(&mut ops, true, false, Vec::new(), None);
        assert_eq!(ops.len(), original_len);
    }

    #[test]
    fn test_optimize_two_subpaths() {
        let mut ops = Ops::new();
        ops.move_to(0.0, 0.0, 0.0, None);
        ops.line_to(10.0, 0.0, 0.0, None);
        ops.move_to(100.0, 0.0, 0.0, None);
        ops.line_to(100.0, 10.0, 0.0, None);
        ops.move_to(10.0, 0.0, 0.0, None);
        ops.line_to(10.0, 10.0, 0.0, None);
        optimize_travel(&mut ops, true, false, Vec::new(), None);
        assert!(ops.len() >= 4);
    }
    ...
    #[test]
    fn test_optimize_with_workpieces() {
        let mut ops = Ops::new();
