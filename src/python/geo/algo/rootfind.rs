...
fn secant_py(
    ...
    let (r, s, i) =
        rootfind::solve_secant(|x| call_f(&f, x), x0, x1, tol, max_iter);
    Ok((r, format!("{s:?}"), i))
}
...
#[pyfunction(name = "solve_illinois")]
fn illinois_py(
    f: Bound<'_, PyAny>,
    lo: f64,
    hi: f64,
    ...
    let (r, s, i) =
        rootfind::solve_illinois(|x| call_f(&f, x), lo, hi, tol, max_iter);
    Ok((r, format!("{s:?}"), i))
}
...
#[pyfunction(name = "solve_secant_tracked")]
fn secant_tracked_py(
    ...
    let (r, s, i, e) = rootfind::solve_secant_tracked(
        |x| call_f(&f, x),
        x0,
        x1,
        tol,
...
#[pyfunction(name = "solve_illinois_tracked")]
fn illinois_tracked_py(
    ...
    let (r, s, i, e) = rootfind::solve_illinois_tracked(
        |x| call_f(&f, x),
        lo,
        hi,
        tol,
